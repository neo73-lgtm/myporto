                  ...
                  >
                    ...
                    <div className="flex justify-center gap-0.5 sm:gap-1 mb-3 sm:mb-4">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <FaStar key={i} className={i < t.rating ? 'text-amber-400' : 'text-slate-200 dark:text-slate-700'} size={16} />
                      ))}
                    </div>
                    <p className="text-sm sm:text-base text-slate-500 dark:text-slate-400 italic leading-relaxed mb-4 sm:mb-6 max-w-xl mx-auto">
                      &ldquo;{lang === 'en' && t.textEn ? t.textEn : t.text}&rdquo;
                    </p>
                    <div>
                      <h4 className="font-semibold text-slate-900 dark:text-white text-sm sm:text-base">{t.name}</h4>
                      <p className="text-xs sm:text-sm text-slate-400 dark:text-slate-500">{t.role} at {t.company}</p>
                    </div>
                  </motion.div>
                )
              )}
