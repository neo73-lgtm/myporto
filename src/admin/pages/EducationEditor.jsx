...
export default function EducationEditor() {
  ...
  useEffect(() => {
    setItems(education.map(p => ({ ...p, _id: Date.now() + Math.random() })));
    setExpanded(new Set());
  }, [education]);

  const addItem = () => {
    const id = Date.now() + Math.random();
    setItems((prev) => [{ _id: id, degree: '', institution: '', location: '', period: '', description: '', gpa: '', technologies: [] }, ...prev]);
  ...
  const handleSave = async () => {
    ...
    const { error: delError } = await supabase.from('education').delete().neq('id', 0);
    if (delError) {
    ...
    if (rows.length > 0) {
      const { error: insError } = await supabase.from('education').insert(rows);
      if (insError) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white">Education</h1>
      ...
      <div className="space-y-3">
        ...
        {items.map((item) => {
          ...
          const summary = [item.degree, item.institution].filter(Boolean).join(' @ ') || 'Item baru';
          return (
          <div key={item._id} data-item-id={item._id} className="card overflow-hidden">
            ...
            >
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  if (window.confirm('Hapus pendidikan ini?')) removeItem(item._id);
                }}
            ...
            <div className="px-4 sm:px-5 pb-4 sm:pb-5 space-y-3 border-t border-slate-100 dark:border-slate-700">
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                <input
                  data-autofocus
                  value={item.degree}
                  onChange={(e) => updateItem(item._id, 'degree', e.target.value)}
                ...
                <input
                  ...
                  onChange={(e) => updateItem(item._id, 'institution', e.target.value)}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <input
                  ...
                  className="px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900/50 text-sm"
                  placeholder="Periode (contoh: 2017 - 2021)"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-[10px] sm:text-xs font-medium text-slate-500 mb-1">
                    <FaStar size={10} className="inline mr-1 text-amber-500" />
                    GPA
                  </label>
                  <input
                    value={item.gpa}
                    onChange={(e) => updateItem(item._id, 'gpa', e.target.value)}
